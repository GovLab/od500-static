/*jshint maxstatements:100, browser:true*/
/*globals d3, $, statesForMap, chartTitleText, queue, country*/
(function(){
    ...
    $('#filters-category').on( 'click', 'button', function( event ) {
        ...
        if ($(this).hasClass('s-active')) { //already active, deactivate
            ...
            //remove all previous filters of the kind
            $("#filters-category button").each(function() {
            ...
            filters.push(filtr);
            window.console.log(filters.join(''));
            //re-draw isotopes
    ...
    $('#filters-agency').on( 'click', 'button', function( event ) {
        ...
        if (filtr === "*") {
            //remove all previous filters of the kind
            $("#filters-agency button").each(function() {
                var i = filters.indexOf($(this).attr('data-filter'));
                if( i > -1) {
                    filters.splice(i, 1);
                }
    ...
    // var userData = document.getElementById('hidden').textContent
    //                        .replace('<','').replace('>','');
    ...
    var createChart = function(){
        ...
        this.mainChart = function() {
            ...
            var margin = {top: marginTop, right: marginRight,
            ...
            bkgdColor.setAttribute("style","background-color:" +
                                   backgroundColor + ";margin:0px;" +
                                   "font-size:12px;");

            var elem = document.getElementById("chart");
            elem.setAttribute("style","width:40% height:" + h +
                              "px; margin: 0px;");
            ...
            group.attr('transform', 'translate(' + margin.left + ', ' +
                       margin.top + '),scale(' + size + ', ' + size + ')');
            ...
            var color = d3.scale.threshold()
                .domain([1, 2, maxVal/7, 2*maxVal/7, 3*maxVal/7, 4*maxVal/7,
                        5*maxVal/7, 6*maxVal/7, maxVal+1 ])
                .range(colors);
            ...
            function ready(error, us, _data) {
                ...
                var states = group.selectAll('.states')
                    .data(data, function(d) {
                        return (d && d.STATE) || d3.select(this).attr("name");
                    })
                    ...
                    .on("mouseover", function(d, i) {
                        ...
                        hover.html(d.STATE + "<br><span style='font-size:" +
                                   hoverValueSize + "; font-family:" +
                                   hoverValueFont + ";color:" +
                                   hoverValueColor + ";'>" +
                                   numberFormat(d.VALUE) + "</span>");
                    })
                    ...
                    .on("mousemove", function(d, i) {
                        if (hoverStates === true) {
                            hover.style("left", (d3.event.pageX) + 20 + "px")
                                 .style("top", (d3.event.pageY) + "px");
                        }
                    });
                ...
                states.on('click', function(s) {
                    ...
                    //if clicking on the same state, remove active class and
                    //remove filter from filters
                    if(this.attributes['class'].value === "states s-active") {
                        filters.splice(filters.indexOf('.'+s.abbrev), 1);
                        d3.select(this)
                            .attr("class", "states")
                            .style("stroke", lineColor)
                            .style("stroke-width", lineThickness);
                    } else { //state previously inactive, activate, and
                             //remove from filter list
                        d3.select(this).attr("class", "states s-active");
                        d3.selectAll('.states')
                            .each(function(d) {
                                var i = filters.indexOf('.'+d.abbrev);
                                //remove all states in filter array
                                if (i > -1) {
                                    filters.splice(i, 1);
                                }
                            })
                            //remove active class from other states
                            .filter(function(d, i) { return  d !== s; })
                            .attr("class", "states")
                            .style("stroke", lineColor)
                            .style("stroke-width", lineThickness);
                        filters.push(filtr);
                    }
                    d3.select(".states.s-active").style("stroke", "black")
                                                 .style("stroke-width", 4);
                    $container.isotope({ filter: filters.join('') });
                });

                //clear filters
                $('#legend').on("click", ".clear-all-filters", function() {
                    filters = [];
                ...
                var cxpos, ctextanchor;

                if (chartTitleXLoc === "Left") {
                    cxpos = margin.left;
                    ctextanchor = "start";
                }
                if (chartTitleXLoc === "Center") {
                    cxpos = (w - margin.right - margin.left)/2 + margin.left;
                    ctextanchor = "middle";
                }
                if (chartTitleXLoc === "Right") {
                    cxpos = (w - margin.right - margin.left) + margin.left;
                    ctextanchor = "end";
                }
